...
function buildLocationTrees(triple) {
  ...
  if (!treeState.nodes.has(src)) {
    treeState.nodes.set(src, { id: src, parents: /* @__PURE__ */ new Map() });
  }
  const srcNode = treeState.nodes.get(src);
  if (!treeState.nodes.has(tgt)) {
    treeState.nodes.set(tgt, { id: tgt, parents: /* @__PURE__ */ new Map() });
  }
  const tgtNode = treeState.nodes.get(tgt);
  treeState.leaves.add(src);
  treeState.leaves.delete(tgt);
  srcNode?.parents.set(tgt, tgtNode);
  return [triple];
}
...
function ThingMetadata() {
  return {
    view(vnode) {
      ...
      const [thing] = things;
      if (thing.feature) {
        metadata["Place Features"] = (0, import_mithril32.default)(FeaturesList, { urns: setify(thing.feature), services });
      }
      if (thing.contains) {
        metadata["Contains"] = (0, import_mithril32.default)(PlacesList, { services, urns: setify(thing.contains) });
      }
      if (thing.unescoId) {
...
function AlbumsApp() {
  return {
    ...
    view() {
      return (0, import_mithril33.default)("body", [
...
//# sourceMappingURL=app.2d4a5535c9.js.map
