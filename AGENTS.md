...
The backend (mirror) selects one cover photo per top-level listing type (bird, mammal, reptile, amphibian, fish, insect, plane, train, car, place, country) via `ListingCoverReader` and publishes a `cover` triple: `urn:ró:photo:<id> cover urn:ró:listing:<type>`. For place and country the best landscape photo is preferred (falling back to highest-rated); all other types use highest-rated only. The listings page calls `services.readCategoryCover(type)` which does a direct triple lookup — no scoring at runtime.
