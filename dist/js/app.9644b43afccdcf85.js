...
`);n=a.pop()??"";for(let p of a){let m=t.parse(p);m!==void 0&&(yield m)}}if(n.length>0){let o=t.parse(n);o!==void 0&&(yield o)}}};var Be="photos";var je={photos:"photos",albums:"albums",album:"album",metadata:"metadata",about:"about",videos:"videos",thing:"thing",listing:"listing"},$=class{static UNESCO="unesco";static BIRD="bird";static MAMMAL="mammal";static REPTILE="reptile";static FISH="fish";static INSECT="insect";static AMPHIBIAN="amphibian";static GEONAME="geoname";static PHOTO="photo";static ALBUM="album";static VIDEO="video";static COUNTRY="country"},g=class{static SUBJECT="subject";static LOCATION="location";static LONGITUDE="longitude";static LATITUDE="latitude";static COUNTRY="country";static FLAG="flag";static RATING="rating";static NAME="name";static BIRDWATCH_URL="birdwatchUrl";static WIKIPEDIA="wikipedia";static CREATED_AT="createdAt";static SEASON="season";static F_STOP="f_stop";static FOCAL_LENGTH="focalLength";static MODEL="model";static EXPOSURE_TIME="exposureTime";static ISO="iso";static WIDTH="width";static HEIGHT="height";static THUMBNAIL_URL="thumbnailUrl";static PNG_URL="pngUrl";static MID_IMAGE_LOSSY_URL="midImageLossyUrl";static FULL_IMAGE="fullImage";static POSTER_URL="posterUrl";static VIDEO_URL_1080P="videoUrl1080p";static VIDEO_URL_480P="videoUrl480p";static VIDEO_URL_720P="videoUrl720p";static VIDEO_URL_UNSCALED="videoUrlUnscaled";static YEAR="year"},He=new Set([g.CREATED_AT,g.F_STOP,g.FOCAL_LENGTH,g.MODEL,g.EXPOSURE_TIME,g.ISO,g.WIDTH,g.HEIGHT]),Z=new Set([$.BIRD,$.MAMMAL,$.REPTILE,$.AMPHIBIAN,$.FISH,$.INSECT]),Ve=new Set([g.THUMBNAIL_URL,g.PNG_URL,g.MID_IMAGE_LOSSY_URL,g.FULL_IMAGE,g.POSTER_URL,g.VIDEO_URL_1080P,g.VIDEO_URL_480P,g.VIDEO_URL_720P,g.VIDEO_URL_UNSCALED]);function As(r,t){let{id:e,type:s}=x(t),i=r.search({source:{id:e,type:s},relation:g.NAME}).firstTarget();if(typeof i>"u")return i;if(typeof i!="string")throw new TypeError(`name is not a string: ${i}`);return i}function Ds(r,t){let{id:e,type:s}=x(t),i=r.search({source:{id:e,type:s},relation:[g.LONGITUDE,g.LATITUDE]}).firstObject();if(i)return{longitude:i.longitude,latitude:i.latitude}}function Rs(r){return{name:r.name,minDate:parseInt(r.minDate),maxDate:parseInt(r.maxDate),thumbnailUrl:r.thumbnailUrl,mosaicColours:r.mosaic,id:r.id,photosCount:r.photosCount,flags:r.flags}}function ks(r){return{...r}}function Os(r){return{...r}}function Ms(r,t){return r.search({source:{type:"photo"},relation:"albumId",target:{id:t}}).sources()}function Ls(r,t){return r.search({source:{type:t},relation:"name"}).objects().sort((s,i)=>s.name.localeCompare(i.name))}var T=class{static getName(t,e){return As(t,e)}static getAlbumPhotoSources(t,e){return Ms(t,e)}static getDistinctNames(t,e){return Ls(t,e)}static getGeocoordinates(t,e){return Ds(t,e)}static videoObjects(t){return t.search({source:{type:"video"}}).objects().map(ks)}static photoObjects(t,e={}){return t.search({...e,source:{type:"photo"}}).objects().map(Os).sort((s,i)=>i.createdAt-s.createdAt)}static albumObjects(t){return t.search({source:{type:"album"}}).objects().map(Rs).sort((e,s)=>s.minDate-e.minDate)}},jt=class{static getURL(t,e){let s=T.getGeocoordinates(t,e);if(!s)return;let{longitude:i,latitude:n}=s;if(i&&n){let o=`https://www.google.com/maps?q=${n},${i}`;return l`
    ...
    `}};h([d()],st.prototype,"id",2),h([d()],st.prototype,"image",2),h([d({state:!0})],st.prototype,"sharing",2),h([d({state:!0})],st.prototype,"triples",2);customElements.define("metadata-page",st);var ge=class extends f{connectedCallback(){super.connectedCallback(),A.setIndex(),document.title="About - photos"}render(){return l`
    <div>
    ...
    `}};customElements.define("about-page",ge);var It=class extends f{connectedCallback(){super.connectedCallback(),A.setIndex(),this.updatePageLocation()}updatePageLocation(){let t=T.getName(this.triples,this.urn);if(!t){document.title="Thing - photos";return}document.title=`${C.capitalise(t)} - photos`}isValidImage(t){return t&&t.thumbnailUrl}urnImages(t,e){let i=t.search(e).sources();return Array.from(i).flatMap(n=>{if(n.startsWith("urn:r\xF3")){let c=t.search({source:x(n)}).firstObject();return this.isValidImage(c)?[c]:[]}let o=t.search({source:{id:n,type:"photo"}}).firstObject();return this.isValidImage(o)?[o]:[]})}renderSubjectPhotos(t){return t.sort((e,s)=>s.createdAt-e.createdAt).map((e,s)=>l`
      <app-photo
        id=${e.id.startsWith("urn:")?E(e.id).id:e.id}
        loading="${U.loadingMode(s)}"
        thumbnailUrl="${e.thumbnailUrl}"
        mosaicColours="${e.mosaicColours}"
        imageUrl="${e.fullImage}"></app-photo>`)}renderSubjectAlbums(t,e){let s=this.urnImages(t,e),i=new Set(s.map(o=>o.albumId)),n=o=>{let c=x(o.id);this.dispatchEvent(new CustomEvent("click-album",{detail:{id:c.id,title:o.title??o.name},bubbles:!0,composed:!0}))};return Array.from(i).flatMap(o=>T.albumObjects(this.triples).filter(c=>E(c.id).id===o)).map(o=>{let c=l`
        <photo-album-metadata
            .triples=${this.triples}
            title="${o.name}"
            count="${o.count}"
      ...
      `})}firstPhotographed(t,e){let i=this.urnImages(t,e).sort((n,o)=>n.createdAt-o.createdAt)[0];return i?new Date(parseInt(i.createdAt)).toLocaleDateString("en-IE",{day:"numeric",month:"short",year:"numeric"}):"Unknown"}renderTitle(){let t=T.getName(this.triples,this.urn);if(t)return t;try{let e=w.parseUrn(this.urn),s=decodeURIComponent(e.id);if(e.id==="*")return C.capitalise(e.type);if(e.type===$.COUNTRY){let i=this.triples.search({source:e}).firstObject();return i.flag?`${i.flag} ${t}`:t}return Z.has(e.type)?et.toCommonName(this.triples,s):s}catch{return this.urn}}renderClassification(t){return l`<a href="#/listing/${t}">${C.capitalise(t)}</a>`}getPhotoQueries(t){let e=t;e.id==="*"&&delete e.id;let s=[];if(Z.has(t.type))for(let i of["captivity","wild"]){let o={...t,qs:{context:i}};s.push({label:i,query:{target:o}})}else s.push({label:"default",query:{source:{type:"photo"},target:t}});return s}renderPhotoSection(t){return l`<div>
    ${Object.entries(t).flatMap(([e,s])=>s?s.length===0?[]:e==="default"?[l`
      ...
      `]:[])}
    <div/>`}render(){let t=this.triples,e=w.parseUrn(this.urn),s=e.type,i=t.search({source:x(this.urn)}).firstObject()??{},n=Object.assign({Classification:this.renderClassification(s)});if(i.country&&(n.Country=l`<thing-link .triples=${this.triples} urn=${i.country}></thing-link>`),i.fcodeName){let D=i.fcodeName;n["Place Type"]=l`${C.capitalise(D)}`}Z.has(s)&&(n["First Photographed"]=l`<span>${this.firstPhotographed(t,{target:x(this.urn)})}</span>`);let o=i[g.WIKIPEDIA],c=i[g.BIRDWATCH_URL],a=jt.getURL(t,this.urn),p=x(this.urn);p.id==="*"&&delete p.id;let m=this.getPhotoQueries(x(this.urn)),u={};for(let{query:D,label:pt}of m){let hs=this.urnImages(t,D);u[pt]=this.renderSubjectPhotos(hs)}let b={source:{type:"photo"},target:p},y=this.renderSubjectAlbums(t,b),I=this.renderPhotoSection(u);return l`
      <div>
    ...
    `}};h([d()],It.prototype,"urn",2),h([d({state:!0})],It.prototype,"triples",2);customElements.define("thing-page",It);var Zt=class{static loadingMode(t){return t===0?"auto":"none"}};var te=class extends f{connectedCallback(){super.connectedCallback(),A.setIndex(),document.title="Videos- photos"}render(){let t=T.videoObjects(this.triples);async function*e(){for(let s=0;s<t.length;s++){let i=t[s];s%4===0&&await new Promise(n=>setTimeout(n,0)),yield l`<app-video
          id=${i.id}
    ...
    `}renderThingAlbum(t,e,s,i){let n=fe.chooseBestImage(this.triples,t,e),o=(m,u)=>{let{type:b,id:y}=x(m),I=new CustomEvent("click-thing-album",{detail:{id:`${b}:${y}`,name:u},bubbles:!0,composed:!0});this.dispatchEvent(I)},c=this.triples.search({source:x(n)}).firstObject(),a=x(this.id),p=`${a.type}:${a.id}`;return l`
      <photo-album
    ...
    `}render(){let t=this.triples,e=T.getDistinctNames(t,this.id);return l`
    <section class="album-metadata">
...
lit-html/directive-helpers.js:
   ...
   *)
*/
//# sourceMappingURL=app.9644b43afccdcf85.js.map
