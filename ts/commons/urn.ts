...
export const buildAlbumUrn = (id: string): string =>
  `${URN_PREFIX}:album:${id}`;
/** Build a photo URN from its identifier. */
export const buildPhotoUrn = (id: string): string =>
  `${URN_PREFIX}:photo:${id}`;
/** Build a video URN from its identifier. */
export const buildVideoUrn = (id: string): string =>
  `${URN_PREFIX}:video:${id}`;
/** Build a place URN from a country identifier. */
export const buildCountryUrn = (id: string): string =>
  `${URN_PREFIX}:place:${id}`;
/** Build a trip URN from its identifier. */
export const buildTripUrn = (id: string): string => `${URN_PREFIX}:trip:${id}`;
/** Build a thing URN from a type and identifier pair. */
export const buildThingUrn = (pair: string): string => `${URN_PREFIX}:${pair}`;
