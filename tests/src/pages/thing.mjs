...
await tap.test("Ireland's page has expected number of CDN image links", async (test) => {
  ...
  const imgCount = await page.$$eval(
    "img",
    (imgs) =>
      imgs.filter((img) => img.src.startsWith("https://photos-cdn.")).length,
  );

  test.ok(
    imgCount >= 500,
    `Page should have at least 500 img tags, found ${imgCount}`,
  );
...
await tap.test("At least 450 images use lazy-loading", async (test) => {
  ...
  const lazyCount = await page.$$eval(
    "img",
    (imgs) => imgs.filter((img) => img.loading === "lazy").length,
  );

  test.ok(
    lazyCount >= 450,
    `At least 450 images should use lazy-loading, found ${lazyCount}`,
  );
...
await tap.test("Check geoname images load", async (test) => {
  ...
  test.ok(
    imgCount >= 2,
    `Page should have at least 2 img tag, found ${imgCount}`,
  );
...
await tap.test("Hohenzollernbrücke page title is correct", async (test) => {
  ...
  test.equal(
    title,
    "Hohenzollernbrücke",
    "Page title should be 'Hohenzollernbrücke'",
  );
...
await tap.test("Hohenzollernbrücke page has Wikipedia link", async (test) => {
  ...
  const linkHandle = await page.$(
    'a[href="https://en.wikipedia.org/wiki/Hohenzollern_Bridge"]',
  );
  ...
  if (linkHandle) {
    ...
    test.equal(
      linkText,
      "[wikipedia]",
      "Wikipedia link text should be '[wikipedia]'",
    );
  }
...
await tap.test("Hohenzollernbrücke page has Maps link", async (test) => {
  ...
  const mapsLinkHandle = await page.$(
    'a[href="https://www.google.com/maps?q=50.94142,6.96535"]',
  );
